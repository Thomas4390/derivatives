...
HESTON_PARAMETERS = [
    ...
    ParameterSpec(
        name="theta",
        display_name="Long-Run Variance (σ²)",
        default=0.04,
        min_value=0.001,
        max_value=1.0,
        step=0.01,
...
MERTON_PARAMETERS = [
    ...
    ParameterSpec(
        name="alpha_j",
        display_name="Mean Jump Size (α_J)",
        default=-0.1,
        min_value=-0.5,
        max_value=0.5,
        step=0.01,
...
MODEL_REGISTRY: dict[str, ModelSpec] = {
    ...
    "heston": ModelSpec(
        ...
        equation_vol=r"dV = \kappa(\sigma^2 - V) \, dt + \alpha \sqrt{V} \, dW_V",
        equation_cf=r"\varphi(u) = \exp\!\bigl(C(u,T) + D(u,T)\,v_0 + iu\ln S_0\bigr), \quad d = \sqrt{(\kappa - i\rho\alpha u)^2 + \alpha^2(iu + u^2)}",
        equation_mc=r"\begin{aligned} S_{t+\Delta t} &= S_t \exp\!\left[(r-q-\tfrac{V_t}{2})\Delta t + \sqrt{V_t\Delta t}\, Z_1\right] \\ V_{t+\Delta t} &= V_t + \kappa(\sigma^2 - V_t)\Delta t + \alpha\sqrt{V_t\Delta t}\, Z_2, \quad \mathrm{corr}(Z_1,Z_2) = \rho \end{aligned}",
        description="Mean-reverting stochastic variance. Captures volatility smile.",
        feller_condition=r"2\kappa\sigma^2 > \alpha^2",
    ),
    "merton": ModelSpec(
        key="merton",
    ...
    "bates": ModelSpec(
        ...
        equation_mc=r"\begin{aligned} S_{t+\Delta t} &= S_t \exp\!\left[(r-q-\lambda k - \tfrac{V_t}{2})\Delta t + \sqrt{V_t\Delta t}\, Z_1\right] \cdot \prod_{j=1}^{N_t} e^{J_j} \\ V_{t+\Delta t} &= V_t + \kappa(\sigma^2 - V_t)\Delta t + \alpha\sqrt{V_t\Delta t}\, Z_2 \end{aligned}",
    ...
    "ngarch": ModelSpec(
        ...
        equation_main=r"r_t = \mu - \frac{1}{2}\sigma_t^2 + \sigma_t z_t",
        equation_vol=r"\sigma_t^2 = \omega + \alpha(\epsilon_{t-1} - \gamma\sigma_{t-1})^2 + \beta \sigma_{t-1}^2",
        equation_mc=r"S_t = S_{t-1}\exp\!\left(\mu - \tfrac{1}{2}\sigma_t^2 + \sigma_t z_t\right), \quad z_t \sim \mathcal{N}(0,1)",
        description="GARCH with leverage effect. Bad news increases vol more than good news.",
        stationarity_condition=r"\alpha(1 + \gamma^2) + \beta < 1",
    ),
    "gjr_garch": ModelSpec(
        key="gjr_garch",
