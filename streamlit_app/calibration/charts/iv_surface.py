...
_SOLVER_COLORSCALES: dict[str, list[list]] = {
    "LM-JAX":   [[0.0, "#7c2d12"], [0.5, "#d97706"], [1.0, "#fcd34d"]],
    "DE":       [[0.0, "#4c1d95"], [0.5, "#7c3aed"], [1.0, "#c4b5fd"]],
    "NM":       [[0.0, "#064e3b"], [0.5, "#059669"], [1.0, "#86efac"]],
    "L-BFGS-B": [[0.0, "#831843"], [0.5, "#db2777"], [1.0, "#f9a8d4"]],
}
...
def _market_points_scatter(
    ...
    *,
    ...
    """Scatter overlay of the actual market IV quotes (the *target*).
    ...
    return go.Scatter3d(
        ...
        z=Z_flat[valid],
        mode="markers",
        name="Market IV quotes",
        # Open red ring, deliberately larger than the model marker so a
        # perfect fit shows the solver's filled diamond *inside* the ring
        # rather than hidden beneath it. ``circle-open`` is the only
        # scatter3d symbol that gives an unfilled outline; ``size=8`` +
        # ``line.width=2`` makes the ring readable at typical chart
        # sizes without bloating the marker beyond the grid spacing.
        marker=dict(
            size=8,
            color=_MARKET_POINT_COLOR,
            symbol="circle-open",
            line=dict(color=_MARKET_POINT_COLOR, width=2),
            opacity=0.95,
        ),
        hovertemplate=(
            f"<b>Market quote</b><br>{x_hover_label}=%{{x:{x_hover_fmt}}}<br>"
...
def _model_points_scatter(
    ...
    *,
    ...
    single legend click toggles surface + dots together.
    """
    M_flat = np.asarray(M).ravel()
    T_flat = np.asarray(T_days).ravel()
...
def render_iv_surface_3d(
    ...
    *,
    ...
    # Industry-standard surface convention: moneyness K/S₀ on the strike axis.
    # Falls back to raw strikes only if spot is unknown.
    if spot is not None and spot > 0.0:
        x_axis_vals = strikes / float(spot)
        x_axis_title = "Moneyness K / S₀ (ratio)"
        x_tickformat = ".2f"
        x_hover_label = "K/S₀"
        x_hover_fmt = ".3f"
    else:
        x_axis_vals = strikes
        x_axis_title = "Strike K (level)"
        x_tickformat = ".0f"
        x_hover_label = "K"
        x_hover_fmt = ".2f"

    M, T = np.meshgrid(x_axis_vals, maturities)
    T_days = T * 365.0
    z = iv_grid * 100.0  # %
    ...
    fig = go.Figure(
        data=[
            go.Surface(
                ...
                colorbar=dict(
                    title=dict(text="Implied Volatility σ (%)", font=dict(family=FONT_FAMILY, color=COLORS["axis"])),
                ...
                showscale=True,
                contours=dict(
                    z=dict(show=True, usecolormap=True, highlightcolor=COLORS["primary"], project_z=True)
                ),
                hovertemplate=(
            ...
            _market_points_scatter(
                M, T_days, z,
                x_hover_label=x_hover_label, x_hover_fmt=x_hover_fmt,
            ),
        ]
    )
...
def render_smile_slices(
    ...
    *,
    ...
    for plot_idx, i in enumerate(order):
        ...
        fig.add_trace(
            go.Scatter(
                ...
                hovertemplate=("M=%{x:.2f}<br>IV=%{y:.2f}%<extra></extra>"),
            )
        )
    fig.add_vline(
        x=1.0,
    ...
    fig.update_xaxes(title="Moneyness K / S₀ (ratio)")
    fig.update_yaxes(title="Implied Volatility σ (%)")
    fig.update_layout(updatemenus=[_smile_maturity_menu(maturities, order, traces_per_maturity=1)])
    return fig
...
def render_smile_slices_overlay(
    ...
    *,
    ...
    """Per-maturity smile with market quotes + one or more model fits.
    ...
    for i in order:
        ...
        fig.add_trace(
            go.Scatter(
                ...
                mode="markers",
                showlegend=False,
                marker=dict(size=8, color=col, line=dict(width=1, color=COLORS["plot"])),
                hovertemplate=("market<br>M=%{x:.2f}<br>IV=%{y:.2f}%<extra></extra>"),
            )
        )
        for slot, (name, grid) in enumerate(models.items()):
            ...
            fig.add_trace(
                go.Scatter(
                    ...
                    hovertemplate=(f"{name}<br>M=%{{x:.2f}}<br>IV=%{{y:.2f}}%<extra></extra>"),
                )
            )
    ...
    for slot, name in enumerate(models):
        fig.add_trace(
            go.Scatter(
                ...
                name=name,
                legendgroup="fit",
                line=dict(color=_MUTED, width=2.2,
    ...
    fig.update_layout(updatemenus=[
        _smile_maturity_menu(
            maturities, order, traces_per_maturity=traces_per_maturity,
...
def render_iv_surface_overlay_3d(
    ...
    maturities: np.ndarray,
    *,
    title: str = "Market vs calibrated surface",
    solver_name: str = "model",
    spot: float | None = None,
) -> go.Figure:
    """One market surface (cool blue) overlaid with N model surfaces.
    ...
    _add_market_surface(
        fig, M, T_days, z_market,
        x_hover_label=x_hover_label, x_hover_fmt=x_hover_fmt,
        legendgroup="market",
    )
    fig.add_trace(_market_points_scatter(
        M, T_days, z_market,
        x_hover_label=x_hover_label, x_hover_fmt=x_hover_fmt,
        legendgroup="market",
    ))
    ...
    for slot, (name, grid) in enumerate(models.items()):
        ...
        fig.add_trace(_model_points_scatter(
            M, T_days, grid * 100.0,
...
def _add_market_surface(
    ...
    *,
    ...
    """Append the cool-blue market reference surface to ``fig``.
    ...
    re-shows the *whole* market layer (surface + ring scatter) at once.
    Default ``None`` keeps single-solver call sites (the animation)
    legend-neutral.
    """
    fig.add_trace(
        go.Surface(
            x=M, y=T_days, z=z_market,
            name="market",
            colorscale=_MARKET_COLORSCALE,
            showscale=False,
            opacity=0.45,
            ...
            hovertemplate=(
                f"market<br>{x_hover_label}=%{{x:{x_hover_fmt}}}<br>"
...
def _surface_legend_annotations(solver_names: list[str]) -> list[dict]:
    """Inline color-coded labels stacked in the top-right corner.

    Replaces the absent legend for ``go.Surface`` traces — the user needs
    to know at a glance which surface is the reference market and which
    is each (calibrated) model. Stacks vertically so multiple solver
    rows stay readable without overlapping.
    """
    annotations = [
        dict(
            ...
            x=0.99, y=0.97,
            xanchor="right", yanchor="top",
            text=f"<span style='color:{_MARKET_LINE}'>● market</span>",
            showarrow=False,
...
def render_iv_surface_animation(
    ...
    *,
    ...
    if not frames:
        ...
        return empty_state_figure("No animation frames available — calibration may have failed.")
    ...
    # Trace 2: market IV quotes (red circles) — static reference.
    fig.add_trace(_market_points_scatter(
        M, T_days, z_market,
        x_hover_label=x_hover_label, x_hover_fmt=x_hover_fmt,
    ))
    ...
    plotly_frames = []
    slider_steps = []
    # Pre-flatten valid mask for the animated model scatter — markers
    # only exist where the market grid has a real quote, so the per-frame
    # update reuses this filter to stay consistent with the static traces.
    M_flat = np.asarray(M).ravel()
    ...
    fig.update_layout(
        ...
        showlegend=False,
        annotations=_surface_legend_annotations([solver_name]),
        updatemenus=[
            dict(
                type="buttons",
                direction="left",
...
def render_returns_summary(prices: np.ndarray, log_returns: np.ndarray, ann: int) -> go.Figure:
    fig = make_subplots(
        rows=1, cols=2,
