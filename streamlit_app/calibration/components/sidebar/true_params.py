...
def render(model_key: str, *, hide_for_real: bool) -> dict[str, float]:
    ...
    for p in spec.params:
        ...
        use_number_input = p.lo < 1e-3
        widget = st.number_input if use_number_input else st.slider
        value = widget(
            p.label,
            min_value=float(p.lo), max_value=float(p.hi),
            value=v, step=float(p.step), format=p.fmt,
            help=p.description,
    ...
    if model_key in ("heston", "bates"):
        ...
        if feller < 0:
            st.warning(
                f"Feller violated: 2κθ − ξ² = {feller:.4f} < 0 — soft penalty active."
            )
    ...
    if model_key in ("garch", "ngarch", "gjr_garch"):
        if out["alpha"] + out["beta"] >= 1.0:
            blocking_error = (
                f"Stationarity violated: α + β = {out['alpha'] + out['beta']:.3f} ≥ 1. "
