...
class FFTEngine(PricingEngine):
    ...
    def greeks(
        ...
        Note: FFTEngine returns only first-order Greeks (delta, gamma, vega, theta, rho).
        For higher-order Greeks, use the GreeksCalculator class which can compute
        second-order (vanna, volga, charm, veta) and third-order (speed, zomma,
        color, ultima) Greeks numerically.
