...
Parameters:
    λ (lam): Jump intensity (expected jumps per year)
    μ_J (alpha_j): Mean of log-jump size
...
def _simulate_merton_paths(
    ...
    n_steps: int,
    z_noise: np.ndarray,
    n_jumps_grid: np.ndarray,
    jump_noise: np.ndarray,
    jump_offsets: np.ndarray,
) -> np.ndarray:
    """
    Numba kernel for Merton jump diffusion path simulation.

    All random draws are pre-generated by the caller:
      - ``z_noise`` (n_paths, n_steps) gaussian for diffusion
      - ``n_jumps_grid`` (n_paths, n_steps) poisson counts
      - ``jump_noise`` (total_jumps,) gaussian innovations for jump sizes
      - ``jump_offsets`` (n_paths * n_steps + 1,) cumulative offsets pointing
        into ``jump_noise`` so each (i, j) cell reads its slice without
        contention with other parallel threads.
    """
    dt = t / n_steps
    sqrt_dt = np.sqrt(dt)

    # Compensator for drift
    k = np.exp(alpha_j + 0.5 * sigma_j * sigma_j) - 1.0

    # Adjusted drift under P-measure
    drift = (mu - lam * k - 0.5 * sigma * sigma) * dt
    ...
    for i in prange(n_paths):
        ...
        for j in range(n_steps):
            ...
            # Jump component (Poisson number of jumps), reading pre-generated
            # gaussian draws from the flat ``jump_noise`` buffer using the
            # cumulative offsets table.
            nj = n_jumps_grid[i, j]
            if nj > 0:
                flat_idx = i * n_steps + j
                offset = jump_offsets[flat_idx]
                jump_sum = 0.0
                for k_idx in range(nj):
                    jump_sum += alpha_j + sigma_j * jump_noise[offset + k_idx]
                log_return += jump_sum
...
def _simulate_merton_terminal(
    ...
    """
    Numba kernel for terminal-only Merton simulation. All random draws
    pre-generated by the caller (see ``_simulate_merton_paths``).
    """
    dt = t / n_steps
    sqrt_dt = np.sqrt(dt)
    ...
    for i in prange(n_paths):
        ...
        for j in range(n_steps):
            ...
            if nj > 0:
                ...
                offset = jump_offsets[flat_idx]
                for k_idx in range(nj):
                    log_return += alpha_j + sigma_j * jump_noise[offset + k_idx]
...
def _draw_merton_noise(
    rng: np.random.Generator,
    lambda_dt: float,
    n_paths: int,
    n_steps: int,
) -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    """Pre-generate all Merton random draws for a thread-safe simulation.

    Returns ``(z_noise, n_jumps_grid, jump_noise, jump_offsets)`` ready to
    feed into the Numba kernels.
    """
    z_noise = rng.standard_normal((n_paths, n_steps))
    n_jumps_grid = rng.poisson(lambda_dt, size=(n_paths, n_steps)).astype(np.int64)
    n_jumps_flat = n_jumps_grid.ravel()
    total_jumps = int(n_jumps_flat.sum())
    jump_offsets = np.empty(n_paths * n_steps + 1, dtype=np.int64)
    jump_offsets[0] = 0
    np.cumsum(n_jumps_flat, out=jump_offsets[1:])
    jump_noise = rng.standard_normal(max(total_jumps, 1))
    return z_noise, n_jumps_grid, jump_noise, jump_offsets
...
class MertonSimulator(BaseSimulator):
    ...
    simulator = MertonSimulator(
        sigma=0.15, lam=0.5, alpha_j=-0.1, sigma_j=0.2
    )
    result = simulator.simulate_paths(s0=100, mu=0.08, t=1.0, n_paths=10000, n_steps=252)
    """

    def __init__(
        self, sigma: float, lam: float, alpha_j: float, sigma_j: float
    ) -> None:
        super().__init__()
    ...
    def _validate_parameters(self) -> None:
        ...
        if self._sigma <= 0:
            raise ValueError(
                f"Diffusion volatility sigma must be positive, got {self._sigma}"
            )
        if self._lambda_j < 0:
            raise ValueError(
                f"Jump intensity lam must be non-negative, got {self._lambda_j}"
            )
        if self._sigma_j < 0:
            raise ValueError(
                f"Jump volatility sigma_j must be non-negative, got {self._sigma_j}"
            )
    ...
    def simulate_paths(
        ...
        lambda_dt = self._lambda_j * (t / n_steps)
        z_noise, n_jumps_grid, jump_noise, jump_offsets = _draw_merton_noise(
            rng, lambda_dt, n_paths, n_steps
        )
...
if __name__ == "__main__":
    ...
    sigma, lam, alpha_j, sigma_j = 0.15, 0.5, -0.1, 0.2
    ...
    for n_paths in path_counts:
        ...
        result = simulate_merton(
            s0, mu, sigma, lam, alpha_j, sigma_j, t, n_paths=n_paths, n_steps=n_steps
        )
