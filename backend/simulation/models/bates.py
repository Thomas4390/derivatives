...
Parameters:
    Heston: κ, θ, ξ, ρ, v0
    Jumps: λ (lam), μ_J (alpha_j), σ_J (sigma_j)
...
def _simulate_bates_paths(
    ...
    Combines Heston variance dynamics with Merton-style jumps under the
    full-truncation scheme. All random draws are pre-generated by the
    caller so the parallel kernel is deterministic across runs:
      - ``z1_noise``, ``z2_noise``: gaussian increments for the correlated
        price/variance Brownian motions.
      - ``n_jumps_grid``: poisson counts per (path, step).
      - ``jump_noise``: flat gaussian buffer for jump sizes.
      - ``jump_offsets``: cumulative offsets indexing ``jump_noise``.
    """
    dt = t / n_steps
    sqrt_dt = np.sqrt(dt)
    ...
    for i in prange(n_paths):
        ...
        for j in range(n_steps):
            ...
            drift = (mu - lam * k - 0.5 * v_plus) * dt
            log_return = drift + sqrt_v * dw_s

            # Jump component (pre-generated)
            nj = n_jumps_grid[i, j]
            if nj > 0:
...
def _simulate_bates_terminal(
    ...
    """
    Numba kernel for terminal-only Bates simulation. All random draws
...
def _draw_bates_noise(
    ...
    """Pre-generate all Bates random draws for a thread-safe simulation."""
    ...
    return z1_noise, z2_noise, n_jumps_grid, jump_noise, jump_offsets
...
class BatesSimulator(BaseSimulator, StochasticVolatilityMixin):
    ...
    def simulate_paths(
        ...
        z1_noise, z2_noise, n_jumps_grid, jump_noise, jump_offsets = (
            _draw_bates_noise(rng, lambda_dt, n_paths, n_steps)
        )
