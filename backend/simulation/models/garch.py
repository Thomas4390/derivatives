...
# Numba resolves module-level constants at compile time, so the imported
# centralised value is captured into the kernel. We alias it locally with the
# same name used inside the kernels for clarity.
...
def _simulate_garch_paths(
    ...
    The pseudo-random innovations are pre-generated by the caller and
    passed via ``z_noise`` (shape ``(n_paths, n_steps)``). This makes the
    parallel kernel deterministic across runs for a fixed seed and avoids
    Numba's thread-local RNG race conditions.
    """
    dt = t / n_steps
    sqrt_dt = np.sqrt(dt)
...
def _simulate_garch_terminal(
    ...
    """
    Numba kernel for terminal-only GARCH simulation. Innovations are
    pre-generated by the caller in ``z_noise`` for reproducibility.
    """
    dt = t / n_steps
    sqrt_dt = np.sqrt(dt)
...
class GARCHSimulator(BaseSimulator, StochasticVolatilityMixin):
    ...
    def __init__(self, sigma0: float, omega: float, alpha: float, beta: float) -> None:
        super().__init__()
...
def estimate_garch_params(
    target_long_run_vol: float, half_life_steps: int = 20, alpha_ratio: float = 0.1
) -> dict[str, float]:
    """
...
def simulate_garch(
    ...
    """
    simulator = GARCHSimulator(sigma0=sigma0, omega=omega, alpha=alpha, beta=beta)
...
if __name__ == "__main__":
    ...
    for n_paths in path_counts:
        ...
        result = simulate_garch(
            s0, mu, sigma0, omega, alpha, beta, t, n_paths=n_paths, n_steps=n_steps
        )
